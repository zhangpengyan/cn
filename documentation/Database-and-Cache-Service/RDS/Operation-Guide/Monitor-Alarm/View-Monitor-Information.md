...
|      IOPS       |    实例每秒读写 IO 的次数，单位：次/秒  |   60 秒/次   |   30 天   |
|   每秒网络流入量  |   实例每秒接受的数据量，单位kbps  |   60 秒/次    |   30 天   |
|   每秒网络流出量  |   实例每秒发送的数据量，单位kbps  |   60 秒/次    |   30 天   |
|   当前连接数      |   实例的活跃连接数、总连接数      |   60 秒/次    |   30 天   |
|   数据请求        |   实例的扫描行数、返回行数        |   60 秒/次    |   30 天   |
|   访问共享锁      |   实例的表锁相关信息              |  60 秒/次     |   30 天   |
|   行共享锁        |   实例的表锁相关信息              |  60 秒/次     |   30 天   |
|   行唯一锁        |   实例的表锁相关信息              |  60 秒/次     |   30 天   |
|   共享更新唯一锁   |  实例的表锁相关信息              |  60 秒/次     |   30 天   |
|   共享锁          |  实例的表锁相关信息              |  60 秒/次     |   30 天   |
|   共享行唯一锁     |  实例的表锁相关信息              |  60 秒/次     |   30 天   |
|   唯一锁          |  实例的表锁相关信息              |  60 秒/次     |   30 天   |
|   访问唯一锁      |  实例的表锁相关信息              |  60 秒/次     |   30 天   |
