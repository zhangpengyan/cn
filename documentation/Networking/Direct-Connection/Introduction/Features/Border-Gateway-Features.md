...
#### **边界网关传播路由排序规则**
 ...
 边界网关的动态路由表内包括所有学习到的传播路由，但由于获取传播路由的途径不一样，需要将前缀一样的路由规则进行比对、筛选出最高优先级的路由规则添加到边界网关的有效路由表中。
 
  边界网关传播路由比对遵循一定的优先级和规则，优先级按照优先级值从小到大逐渐降低，即优先级数值越小、路由优先级别越高。

   ***传播路由内部排序***

   VPC传播路由优先级最高（优先级值缺省为110），相同prefix仅一条、先加入者生效，先加入路由表的vpc传播路由删除后、后续的vpc传播路由再生效；

   EBGP动态路由优先级其次（优先级缺省为120），如果同为EBGP路由，按照如下顺序从上到下优选：
  ...
  IBGP动态路由优先级最低（优先级缺省为200）。

#### **边界网关有效路由规则匹配原则**
  
   京东云边界网关路由优先级值范围为1~256，其中静态路由优先级缺省为100、VPC传播路由优先级缺省为110、EBGP传播路由优先级缺省为120，IBGP传播路由优先级缺省为200。
   
   边界网关有效路由表中仅展示经过比对淘汰后的实际有效路由规则，一些前缀相同的低优先级无效路由不会在有效路由表中展示、但可以在动态路由表中可以查询。

  边界网关进行业务数据转发时，将依据其有效路由表内的路由规则进行逐项匹配、然后按照路由规则的下一跳信息将报文转发到相应的接口或者通道。
  
  ***边界网关有效路由规则匹配顺序***
  
   最长前缀prefix（掩码位数最长）匹配优先；
    
   同样长度前缀prefix，路由优先级不一样时，按照优先级从小到大进行匹配。优先级数值越小、路由优先级别越高，如匹配顺序从前到后为：静态路由->VPC传播路由->EBGP传播路由->IBGP传播路由；

   同样长度前缀prefix，路由优先级一样时，则为等价路由（目前仅静态路由支持等价）。
