...
2.点击左侧导航栏，选择“专线服务 -> 边界网关”，进入用于承载专线连接网络互通的边界网关的详情页面，选择“路由表”页签。
...
如果边界网关已经创建了与IDC互联的通道（如专线通道等），并且运行了BGP协议进行两端动态路由学习，边界网关会将通过BGP学习到的有效路由自动添加到有效路由表内、下一跳为与托管区互联的通道（如专线通道），用户直接在有效路由表内查看即可，不需要用户配置。
