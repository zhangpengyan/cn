# 快速部署基于相同业务的多个监听器
  ...
  假如您的业务需要同时支持多个监听器，并需复用后端服务， 例如，您在后端服务同时支持HTTP和HTTPS访问，可配置HTTP监听和HTTPS监听关联同一个后端服务，实现快速部署。
...
3.点击 **下一步** 进入后端转发配置页签。选择默认后端服务类型为 **新建后端服务**。配置后端服务名称、后端协议、调度算法、连接耗尽超时时间等。
