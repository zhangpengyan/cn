...
#### **边界网关路由表传播路由排序规则**
 ...
 边界网关的动态路由表内包括所有学习到的传播路由，但由于获取传播路由的途径不一样，需要将前缀一样的路由规则进行比对、筛选出最高优先级的路由规则添加到边界网关的有效路由表中。
  ...
  边界网关传播路由比对遵循一定的优先级和规则，优先级按照优先级值从小到大逐渐降低，即优先级数值越小、路由优先级别越高。
   ...
   1，VPC传播路由优先级最高（优先级值缺省为110），相同prefix仅一条、先加入者生效，先加入路由表的vpc传播路由删除后、后续的vpc传播路由再生效；

   2，EBGP动态路由优先级其次（优先级缺省为120），如果同为EBGP路由，按照如下顺序从上到下优选：
