...
| 错误码	| 含义	| 出现原因 | 该怎么办 |
| :- | :- | :- | :- |
|ERROR 400| 域名不存在 | 高防实例中没有配置该域名| 检测配置，添加网站转发域名|
|ERROR 421 | 连接被阻断	| 命中黑名单规则 |	请确认该网站已购买京东云的IP高防服务，且配置已生效。|
|ERROR 493 |	请求重入 |	您的同一访问多次经过京东IP高防的节点或者被CC防护拦截。|	请确认网站配置，是否配置了多重代理服务器，且代理路径发生环路。|
|ERROR 522 | 源站返回错误 |源站返回500，502错误号 | 检测源站服务|
|ERROR 526 |	源站繁忙 | 高防节点回源建连失败或者连接超时	| 检测源站|
|ERROR 527 | 源站返回错误 |源站返回504错误号 | 检测源站服务|
