...
HTTPRequestMethod即HTTP协议请求方式(如POST、GET等，使用全大写字母表示)，并后跟换行符。

#### CanonicalURI是对URI的路径组件根据(RFC 3986)进行编码，路径组件是指以域名后的正斜杠(/)开始，直到第一个(?)、(#)或URI末尾结束。
* 对路径组件编码时不对其中的正斜杠(/)进行编码。
* 如果路径组件为空，则使用正斜杠(/)。
* 路径组件中连续的多个正斜杠会被替换为一个正斜杠。

#### CanonicalQueryString是对URI的查询字符串根据(RFC 3986)进行编码，并后跟换行符。要构建规范查询字符串，步骤如下：
* a、首先按字符代码对**参数名按升序**进行排序，如果参数名称重复，则再**按值升序**进行排序。
* b、然后按照(RFC 3986)对每个**参数名称和值分别进行URI编码**（不要重复编码）。
* c、接着通过从排序列表中的第一个参数名称开始构建规范查询字符串。**对于每个参数，附加URI编码的参数名称，后跟等号字符（=），后跟URI编码的参数值**。对没有值的参数使用空字符串。
* d、在每个参数值之后附加&符号，除了列表中的最后一个值。
* e、如果URI中没有查询字符串，则将规范查询字符串设置为空字符串（''），并添加（'\n'）。

#### CanonicalHeaders是要创建规范的HTTP请求头列表，并后跟换行符。请求头列表是指您要包含在签名请求中的所有HTTP请求头列表。创建步骤如下：
* a、请将所有HTTP请求头名称转换为**小写**，并**删除请求头value中前导空格和尾随空格**；如果请求头value包含连续空格，则转换为单个空格。
* b、通过用字符代码**升序**对请求头进行排序。
* c、使用排序后的小写请求头名称，后跟冒号，然后添加该请求头的value；多个value时，以逗号(,)分隔，并且不要对value进行排序；最后添加('\n')。
...
* a、请将所有HTTP请求头名称转换为**小写**。
* b、通过用字符代码**升序**对请求头进行排序。
* c、将排序后的小写请求头名称使用分号(;)进行分隔，最后添加('\n')。

注意：**SignedHeaders中的请求头与您在CanonicalHeaders字符串中包含的请求头相同**

#### 最后，需要把请求体中的payload做SHA256哈希后，再转为**小写十六进制字符串**进行表示。如果payload为空，则使用空字符串('')作为SHA256哈希的输入。
...
Lowercase(HexEncode(CanonicalRequest))是步骤1生成的标准请求进行**SHA256哈希**后，再表示为**小写十六进制字符串**。
